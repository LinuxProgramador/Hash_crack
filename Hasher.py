...
from hashlib import md5,sha1,sha224,sha384,sha256,sha512,sha3_256,sha3_224,sha3_384,sha3_512,blake2s,blake2b,shake_128,shake_256,pbkdf2_hmac,algorithms_available,new,scrypt
...
""")
       ...
       if not option_chosen_2 in ["1","2"]:
         ...
         system("clear")
         system("python3 ~/Hash_crack/break_bcrypt.py")
         exit(0)
    return
...
WARNING:BE CAREFUL WITH THE NUMBER OF PASSWORDS YOU USE. CAN BE GENERATED, IT CAN REACH UP TO GIGABYTES AND BLOCK THE SYSTEM!!!!!!!
  ...
  def auxiliary_hash_cracking_worker(self,hash_input,password,wpa_psk,ssid,is_fast_mode,crackTimeEstimate,data,select,wait_time):
          ...
          #It's a slow hash
               ...
               self.validation(hash_pbkdf2,hash_input,password,wpa_psk,ssid,is_fast_mode,crackTimeEstimate)
  ...
  def auxiliary_process_wpa_passwords(self,password,ssid,is_fast_mode,crackTimeEstimate,wpa_psk,hash_input):
         ''' Generates a WPA-PSK hash using PBKDF2-HMAC-SHA1 and compares it to the target hash to determine if the password is correct or proceed with a fallback '''
  ...
  def process_wpa_passwords(self,password_list,combined,data,keyBin,wpa_psk,ssid,is_fast_mode,crackTimeEstimate,hash_input):
    ...
    for keyword in password_list:
      if 8 <= len(keyword) <= 63 or validation_str and 8 <= len(backup_password_list) <= 63:
         ...
         if combined == "y":
            password,data = self.validation_combined(password,data,keyclean,keyBin,wpa_psk)
            self.auxiliary_process_wpa_passwords(password,ssid,is_fast_mode,crackTimeEstimate,wpa_psk,hash_input)
  ...
  def crack(self,hash_input,select,is_fast_mode,combined,wait_time):
     ...
     if combined == "y" or select in ["pbkdf2_sha256",'scrypt',"argon2id","bcrypt","DCC2","sha256crypt","sha512crypt","md5crypt","apr1","phpass"] or wait_time == "y" and is_fast_mode != "y":
        crackTimeEstimate = "time unknown"
     wpa_psk = False
