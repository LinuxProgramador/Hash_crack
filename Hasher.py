...
from hashlib import md5,sha1,sha224,sha384,sha256,sha512,sha3_256,sha3_224,sha3_384,sha3_512,blake2s,blake2b,shake_128
  ...
  def main(self):
   ...
   try:
    ...
    if "-sk" in argv:
         ...
         if hash_input:
           if len(hash_input) >= 1 and len(hash_input) <= 1024:
             hash = "shake-128"
             self.cracking_selection(hash_input,hash)
             exit(2)
           else:
